...
def create_cloudformation_template_alerts(alert_yml_data=None, aws_keys=None, deploy=False):
    ...
    for category in alert_yml_data:
        ...
        for monitoring_system in alert_yml_data.get(_category):
            ...
            if _monitoring_system == "AWS":
                ...
                stack_name = "{0}-{1}-alerts-stack.yml".format(_account, category)
                stack_file_path = os.path.join(_path_deployment_plugin, stack_name)
                ...
                if deploy:
                    ...
                    if account_exists:
                        if check_cloudformation_stack(stack_name=stack_name,
                            ...
                            check_cloudformation_stack(stack_name=stack_name,
                                                       aws_access_key_id="",
                                                       aws_secret_access_key="",
                                                       region="")
                    else:
...
def create_cloudformation_stack(cloudformation_path="", stack_name="", bucket_name=None, aws_access_key_id="",
    ...
    try:
        ...
        # If bucket_name exist load cloudformation template from S3 bucket
            ...
            response = cloud_formation.create_stack(
                StackName=stack_name,
                TemplateBody=Utils.read_yml_file(cloudformation_path),
            )
...
# Main function, everything starts here =D
def main(list_alerts_file, password_file_path):
    ...
    aws_keys = get_aws_keys(password_file_path)["AWS"]
